...
const features = [
  ...
  { icon: Search, title: "Content Gap Analysis", description: "Identify which topics, questions, and prompts your content doesn't cover — before your competitors do.", color: "text-green-400", bg: "bg-green-500/10" },
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="fixed top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto flex h-14 items-center px-4">
          <div className="flex items-center gap-2 mr-auto">
            ...
            <span className="hidden sm:inline text-muted-foreground text-sm ml-1">AI Engine Optimization</span>
          </div>
          <nav className="flex items-center gap-4 ml-auto">
            <Link to="/auth/login" className="text-sm text-muted-foreground hover:text-foreground transition-colors">Sign In</Link>
            <Link to="/auth/register" className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
      ...
      <section className="container mx-auto px-4 pt-32 pb-20 text-center">
        ...
        <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight mb-6">
          Your brand in every <span className="gradient-text">AI answer</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link to="/auth/register" className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-base font-semibold text-primary-foreground hover:bg-primary/90 transition-colors shadow-lg shadow-primary/25">
            Start for free <ArrowRight className="h-4 w-4" />
          </Link>
          <Link to="/auth/login" className="inline-flex items-center gap-2 rounded-lg border border-border px-6 py-3 text-base font-semibold hover:bg-accent transition-colors">
            View demo
          </Link>
        </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8">
            <div className="relative flex-shrink-0">
              <svg viewBox="0 0 200 200" className="w-48 h-48">
                <circle cx="100" cy="100" r="85" fill="none" stroke="hsl(217.2 32.6% 17.5%)" strokeWidth="14" />
                <circle cx="100" cy="100" r="85" fill="none" stroke="hsl(217.2 91.2% 59.8%)" strokeWidth="14" strokeLinecap="round" strokeDasharray={`${2 * Math.PI * 85}`} strokeDashoffset={`${2 * Math.PI * 85 * (1 - 0.42)}`} transform="rotate(-90 100 100)" />
              ...
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                    <div className={`h-full rounded-full ${item.score >= 60 ? "bg-green-500" : item.score >= 40 ? "bg-yellow-500" : "bg-red-500"}`} style={{ width: `${item.score}%` }} />
                  </div>
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4">Everything you need for AI visibility</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature) => {
            ...
            return (
              <div key={feature.title} className="rounded-xl border border-border bg-card p-6 card-hover">
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="rounded-2xl bg-gradient-to-br from-primary/20 to-purple-500/20 border border-primary/30 p-12 text-center">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold mb-4">Own your position in the AI era</h2>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto mb-8">
            Companies that invest in AI Engine Optimization today will dominate AI-generated answers for years to come.
          </p>
          <Link to="/auth/register" className="inline-flex items-center gap-2 rounded-lg bg-primary px-8 py-3 text-base font-semibold text-primary-foreground hover:bg-primary/90 transition-colors">
