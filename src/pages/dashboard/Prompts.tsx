...
interface IntentMeta {
  id: LLMIntentType; label: string; desc: string; icon: React.ReactNode; color: string; example: string;
}

const INTENT_META: IntentMeta[] = [
  { id: "seek_explanation", label: "Explain / What is", desc: "User asks the AI to explain or define something", icon: <HelpCircle className="h-4 w-4" />, color: "text-blue-400", example: '"What is AI observability?"' },
  { id: "find_best", label: "Find the best", desc: "User asks for top tools or recommendations", icon: <TrendingUp className="h-4 w-4" />, color: "text-emerald-400", example: '"Best LLM monitoring tools 2026"' },
  { id: "compare", label: "Compare options", desc: "User compares two or more products side-by-side", icon: <Swords className="h-4 w-4" />, color: "text-orange-400", example: '"GAEO vs LangSmith"' },
  { id: "learn_howto", label: "Learn how-to", desc: "User wants step-by-step guidance", icon: <BookOpen className="h-4 w-4" />, color: "text-yellow-400", example: '"How to monitor LLM apps in production?"' },
  { id: "find_alternative", label: "Find alternatives", desc: "User looking for alternatives to a tool", icon: <Hash className="h-4 w-4" />, color: "text-purple-400", example: '"LangSmith alternatives"' },
  { id: "troubleshoot", label: "Troubleshoot / Fix", desc: "User diagnosing a problem or seeking a fix", icon: <Wrench className="h-4 w-4" />, color: "text-red-400", example: '"Why is my LLM giving wrong answers?"' },
];
    ...
    <div className="p-6 lg:p-8 space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between gap-4">
        <div className="page-header pb-0">
          <h1>AI Discovery Prompt Engine</h1>
          <p>Manage the prompts users ask AI systems — organised by interaction intent</p>
        </div>
        <button onClick={handleDiscover} disabled={isDiscovering}
          className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 px-5 py-2.5 text-sm font-bold text-white hover:opacity-90 transition-all disabled:opacity-60 shadow-lg shadow-primary/20"
        >
      ...
      <div className="grid grid-cols-3 gap-5">
        {[
          { value: totalPrompts, label: "Total Prompts", color: "text-foreground" },
          { value: coveredPrompts, label: "Content Covered", color: "text-emerald-400" },
          { value: totalPrompts - coveredPrompts, label: "Coverage Gaps", color: "text-red-400" },
        ].map((stat, i) => (
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="stat-card text-center">
      ...
      {tab === "database" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-1 mb-3">User Interaction Intents</p>
              ...
              return (
                <button key={meta.id} onClick={() => setSelectedIntent(meta.id)}
                  className={`w-full flex items-center gap-3 rounded-2xl border p-4 text-left transition-all ${
                    selectedIntent === meta.id ? "border-primary/30 bg-primary/5 shadow-sm" : "border-border/60 hover:border-primary/20 bg-card/80"
                  }`}
                >
                  <div className={meta.color}>{meta.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm">{meta.label}</p>
                    <p className="text-xs text-muted-foreground tabular-nums">{count} prompts · {covered} covered</p>
                  </div>
                  {count > 0 && (
          ...
          <div className="lg:col-span-2 section-card">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-border/60">
              ...
              <div>
                <h2 className="font-bold">{activeIntentMeta.label}</h2>
                <p className="text-xs text-muted-foreground">{activeIntentMeta.desc}</p>
              </div>
              <span className="ml-auto text-xs text-muted-foreground tabular-nums">{intentPrompts.length} prompts</span>
            </div>
            <div className="px-6 py-3 border-b border-border/60 bg-muted/10">
              <p className="text-xs text-muted-foreground">Example: <span className="italic text-foreground/70">{activeIntentMeta.example}</span></p>
            </div>

            {intentPrompts.length === 0 ? (
              <div className="p-12 text-center text-muted-foreground">
                <p className="text-sm font-medium">No prompts for this intent yet.</p>
                <p className="text-xs mt-1">Use "Discover Prompts" or add one manually below.</p>
              </div>
            ) : (
              <div className="divide-y divide-border/60">
                {intentPrompts.map((prompt) => (
                  <div key={prompt.id} className="flex items-center gap-3 px-6 py-3.5 hover:bg-accent/20 transition-colors">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">{prompt.text}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {prompt.covered ? (
                        <span className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/15 px-2.5 py-0.5 rounded-full flex items-center gap-1 font-medium">
                          <CheckCircle2 className="h-3 w-3" /> Covered
                        </span>
                      ) : (
                        <span className="text-xs bg-red-500/10 text-red-400 border border-red-500/15 px-2.5 py-0.5 rounded-full font-medium">Gap</span>
                      )}
                      <button onClick={() => toast.success("Added to simulation queue")}
                        className="rounded-lg border border-border/60 p-1.5 text-muted-foreground hover:text-foreground hover:bg-accent transition-all" title="Run in simulation"
                      >
            ...
            <div className="px-6 py-4 border-t border-border/60 flex items-center gap-2">
              <input value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddCustomPrompt()}
                ...
                className="flex-1 rounded-xl border border-input bg-background/80 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring/50 transition-all"
              />
              <button onClick={handleAddCustomPrompt} disabled={!customPrompt.trim()}
                className="rounded-xl border border-border/60 px-3 py-2.5 hover:bg-accent disabled:opacity-60 transition-all"
              >
      ...
      {tab === "simulate" && (
        <div className="section-card p-6 space-y-5">
          <h2 className="font-bold">Quick Prompt Simulation</h2>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} placeholder="What are the best AI observability tools?"
                className="w-full rounded-xl border border-input bg-background/80 pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring/50 transition-all"
              />
            </div>
            <button onClick={() => { toast.success("Simulation queued — check LLM Simulation page"); setCustomPrompt(""); }} disabled={!customPrompt.trim()}
          ...
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-bold mb-2">Prompts from your database:</p>
            {allPrompts.slice(0, 6).map((p) => (
              <button key={p.id} onClick={() => setCustomPrompt(p.text)}
                className="w-full text-left text-sm text-muted-foreground hover:text-foreground flex items-center gap-2.5 px-4 py-2.5 rounded-xl hover:bg-accent/40 transition-all"
              >
