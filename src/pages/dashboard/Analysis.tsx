...
const DEMO_REPORT: Partial<AnalysisReport> = {
  ...
  content_roadmap: [
    ...
    { priority: 3, title: "AI Observability vs Traditional Monitoring", type: "comparison", target_prompts: ["ai observability vs monitoring"], expected_impact: "Covers comparison prompts", score_impact: { metric: "ecosystem_coverage", estimated_gain: 8 }, outline: ["Key differences", "When to use each", "Feature comparison", "Migration guide"] },
  ],
};
...
export default function AnalysisPage() {
  ...
  const scores: Record<string, number> = {
    overall: 42, entity_clarity: 65, category_ownership: 52, educational_authority: 45,
    prompt_coverage: 28, comparison_coverage: 15, ecosystem_coverage: 38, external_authority: 30,
    community_signal: 20, consistency: 72, structure_quality: 58,
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleRun} disabled={running} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-60 transition-colors">
          {running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />} Re-analyze
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-xl border border-border bg-card p-6 flex flex-col items-center">
          ...
          <div className="mt-4 w-full space-y-1 text-sm">
            <div className="flex justify-between text-muted-foreground"><span>Industry Average</span><span className="font-medium text-foreground">51 / 100</span></div>
            <div className="flex justify-between text-muted-foreground"><span>Top Performers</span><span className="font-medium text-green-400">78 / 100</span></div>
            <div className="flex justify-between text-muted-foreground"><span>Your Score</span><span className="font-medium text-orange-400">42 / 100</span></div>
          </div>
        </div>
        <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6">
          ...
          <div className="space-y-3">
            {SCORE_DIMENSIONS.map((dim) => (
              <ScoreBar key={dim.key} label={dim.label} score={scores[dim.key] || 0} description={dim.description} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1 rounded-lg border border-border bg-muted/30 p-1 w-fit">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`rounded-md px-4 py-1.5 text-sm font-medium transition-colors ${activeTab === tab.id ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {activeTab === "recommendations" && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">Top recommendations sorted by impact.</p>
          {report.recommendations?.map((rec, i) => (
            <div key={i} className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
              <div className={`h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold ${rec.impact >= 8 ? "bg-red-500/20 text-red-400" : rec.impact >= 6 ? "bg-orange-500/20 text-orange-400" : "bg-yellow-500/20 text-yellow-400"}`}>{i + 1}</div>
              <div className="flex-1"><p className="text-sm font-medium">{rec.action}</p></div>
      ...
      {activeTab === "gaps" && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">High-importance topics not covered by your content.</p>
            ...
            <div key={i} className="flex items-start gap-4 rounded-xl border border-border bg-card p-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${gap.suggested_format === "guide" ? "bg-blue-500/15 text-blue-400" : "bg-green-500/15 text-green-400"}`}>{gap.suggested_format}</span>
                </div>
                <p className="text-xs text-muted-foreground">Suggested: <span className="text-foreground">{gap.suggested_title}</span></p>
              </div>
      ...
      {activeTab === "roadmap" && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Your prioritized 90-day content roadmap.</p>
          {report.content_roadmap?.map((item) => (
            <div key={item.priority} className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-start gap-3">
                <div className="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0 text-sm font-bold text-primary">{item.priority}</div>
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-semibold">{item.title}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${item.type === "guide" ? "bg-blue-500/15 text-blue-400" : item.type === "comparison" ? "bg-orange-500/15 text-orange-400" : "bg-green-500/15 text-green-400"}`}>{item.type}</span>
                  </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {item.outline.map((section, i) => (
                          <span key={i} className="text-xs flex items-center gap-0.5 text-muted-foreground">
                            {i > 0 && <ChevronRight className="h-3 w-3" />}{section}
                          </span>
                        ))}
                      </div>
                    </div>
