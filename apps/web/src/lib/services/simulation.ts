  ...
  try {
    ...
    for (const prompt of prompts) {
      for (const { provider, model } of models) {
        tasks.push(
          runSingleSimulation(jobId, job.projectId, prompt, provider as 'anthropic' | 'openai' | 'google', model, productName)
        )
      }
    }
...
2. If mentioned, what rank/position (1=first recommendation, 2=second, etc., null if not mentioned)
3. The exact text context around the mention (or null)
4. Sentiment toward "${productName}" from -1.0 to 1.0
5. Confidence score that "${productName}" is genuinely recommended 0.0-1.0
6. List of all brands/products/tools mentioned
7. List of competitor products mentioned (not "${productName}")
8. List of any URLs/citations in the response

Respond with JSON:
{
  "productMentioned": true/false,
  "mentionRank": 1 or null,
  "mentionContext": "...text snippet..." or null,
