...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = await prisma.user.create({
      data: { email, fullName, hashedPassword, isVerified: true },
    })
    ...
    const token = await signToken({ sub: user.id, email: user.email, workspaceId: workspace.id })

    log.info('User registered', { userId: user.id, email: user.email, workspaceId: workspace.id })
