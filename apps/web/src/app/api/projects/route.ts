...
export async function GET(req: NextRequest) {
  try {
    ...
    log.error('Failed to fetch projects', { error: (err as Error).message })
...
export async function POST(req: NextRequest) {
  try {
    ...
    log.info('Project created', { projectId: project.id, userId: auth.sub, name: project.name })
    return Response.json({ project }, { status: 201 })
  } catch (err: unknown) {
    if ((err as Error).name === 'AuthError') return authErrorResponse()
    log.error('Failed to create project', { error: (err as Error).message })
