...
const log = logger('api/monitoring/jobs')
...
export async function GET(req: NextRequest) {
  try {
    ...
    log.error('Failed to fetch monitoring jobs', { error: (err as Error).message })
...
export async function POST(req: NextRequest) {
  try {
    ...
    log.info('Monitoring job created', { userId: auth.sub, projectId, jobId: job.id, name })
    return Response.json({ job }, { status: 201 })
  } catch (err: unknown) {
    if ((err as Error).name === 'AuthError') return authErrorResponse()
    log.error('Failed to create monitoring job', { error: (err as Error).message })
