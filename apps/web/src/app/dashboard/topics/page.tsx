...
import { AlertCircle, CheckCircle2, Map, RefreshCw } from "lucide-react";

// Mock topic graph data
const MOCK_NODES = [
  // Root
...
const MOCK_EDGES = [
  { source: "root", target: "monitoring" },
  { source: "root", target: "evaluation" },
  { source: "root", target: "debugging" },
  { source: "root", target: "performance" },
  { source: "root", target: "security" },
  { source: "monitoring", target: "tracing" },
  { source: "monitoring", target: "logging" },
  { source: "evaluation", target: "benchmarks" },
  { source: "evaluation", target: "testing" },
  { source: "security", target: "prompt-injection" },
  { source: "performance", target: "latency" },
  { source: "performance", target: "cost" },
];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Your content coverage across the AI Observability topic landscape
          </p>
        </div>
            ...
            {filteredNodes.map((node) => (
              <button
                ...
                className={`w-full flex items-center gap-2 rounded-lg p-2.5 text-left transition-colors ${
                  selectedNode?.id === node.id ? "bg-primary/15 border border-primary/30" : "hover:bg-accent/50"
                }`}
              >
                {node.covered ? (
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {selectedNode.covered
                  ? "This topic is covered by your content."
                  : "Content gap — this topic is not covered by your content."}
              </p>
            </div>
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${
              selectedNode.covered ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"
            }`}>
          ...
          {!selectedNode.covered && (
            <div className="mt-3 pt-3 border-t border-border">
              ...
              <p className="text-sm text-muted-foreground">
                Create content covering &ldquo;{selectedNode.label}&rdquo; to fill this gap. Suggest creating a comprehensive guide or FAQ article.
              </p>
            </div>
          )}
        </div>
